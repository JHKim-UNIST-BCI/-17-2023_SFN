...
class Synapse(Module):
    ...
    def cal_post_input_delay(self, pre_spike_times: Tensor, buffer_size: int = 50) -> Tensor:
        ...
        self.psp = self.psp * \
            math.exp(-self.dt / self.tau_psp) + spikes_with_delay
        delayed_post_input = torch.einsum('ij,ij->i', self.weights, self.psp)

        # print(self.psp)
        return delayed_post_input

    def cal_post_input_delay_old(self, pre_spike_times: Tensor, buffer_size: int = 50) -> Tensor:
        synaptic_delay_transpose = (buffer_size-1) - self.delays.t().long()
        spikes_with_delay = torch.gather(
            pre_spike_times, 1, synaptic_delay_transpose).t()
        delayed_post_input = torch.einsum(
            'ij,ij->i', self.weights, spikes_with_delay)
