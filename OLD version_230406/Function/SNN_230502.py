...
class SNNModel(nn.Module):
    ...
    def feedforward(self, stim):
        ...
        SA_stim = stim
        RA_stim = torch.abs(torch.diff(stim, dim=2)) * 100
    ...
    def process_stim(self,
        ...
        for i in range(1, stim_len):
            ...
            SA_layers[2].update(SA_rf_input_PN1 * 2  - SA_rf_input_PN2 * 2)
            ...
            RA_layers[2].update(RA_rf_input_PN1 * 2 - RA_rf_input_PN2 * 2)
            ##############################################################
            ...
            CN_IN_RA_rf_input_resized = self.process_input(
                CN_IN_RA_rf_input, self.rf_sizes)
            ...
            CN_layers[1].update((CN_PN_SA_rf_input + CN_PN_RA_rf_input_resized) * 2 - CN_IN_rf_input * 2)
            ##############################################################
    ...
    def process_input(self, input_tensor, rf_sizes):
        ...
        for h, w in rf_sizes:
            ...
            input_part = F.interpolate(input_part.view(1, 1, h, w), size=(
                h, w), mode="bilinear", align_corners=False).view(h, w)
